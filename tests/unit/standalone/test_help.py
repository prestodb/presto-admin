...
class TestStandaloneHelp(BaseMainCase):
    ...
    def setUp(self, mode_mock):
        ...
        reload(prestoadmin)
        self.short_help_path = os.path.join(
            'resources', 'standalone-help.txt')
        self.extended_help_path = os.path.join(
            'resources', 'standalone-extended-help.txt')

    def get_extended_help_path(self):
        return '/resources/standalone-extended-help.txt'

    def test_standalone_help_text_short(self):
        self._run_command_compare_to_file(
            ["-h"], 0, self.short_help_path)

    def test_standalone_help_text_long(self):
        self._run_command_compare_to_file(
            ["--help"], 0, self.short_help_path)

    def test_standalone_help_displayed_with_no_args(self):
        self._run_command_compare_to_file(
            [], 0, self.short_help_path)

    def test_standalone_extended_help(self):
        self._run_command_compare_to_file(
            ['--extended-help'], 0, self.extended_help_path)
