...
class TestCollect(BaseProductTestCase):
    ...
    def test_collect_logs_basic(self):
        ...
        self.assert_path_exists(self.cluster.get_master(), OUTPUT_FILENAME_FOR_LOGS)
        self.assert_path_exists(self.cluster.get_master(), TMP_PRESTO_DEBUG)

        downloaded_logs_location = path.join(TMP_PRESTO_DEBUG, 'logs')
        self.assert_path_exists(self.cluster.get_master(), downloaded_logs_location)

        for host in self.cluster.all_internal_hosts():
            host_log_location = path.join(downloaded_logs_location, host)
            self.assert_path_exists(self.cluster.get_master(), host_log_location)

        admin_log = path.join(downloaded_logs_location, PRESTOADMIN_LOG_NAME)
        self.assert_path_exists(self.cluster.get_master(), admin_log)
    ...
    def _test_basic_system_info(self, actual, coordinator=None, hosts=None):
        ...
        self.assert_path_exists(self.cluster.get_master(), OUTPUT_FILENAME_FOR_SYS_INFO)
        self.assert_path_exists(self.cluster.get_master(), TMP_PRESTO_DEBUG)

        downloaded_sys_info_loc = path.join(TMP_PRESTO_DEBUG, 'sysinfo')
        self.assert_path_exists(self.cluster.get_master(), downloaded_sys_info_loc)

        conn_file_name = path.join(downloaded_sys_info_loc, 'connector_info.txt')
        self.assert_path_exists(self.cluster.get_master(), conn_file_name)
        ...
        self.assert_path_exists(self.cluster.get_master(), coord_system_info_location)

        coord_catalog_info_location = path.join(coord_system_info_location, 'catalog')
        self.assert_path_exists(self.cluster.get_master(), coord_catalog_info_location)
        self.assert_path_exists(self.cluster.get_master(), path.join(coord_catalog_info_location, 'tpch.properties'))

        # collected worker info
        slave0_system_info_loc = path.join(downloaded_sys_info_loc, self.cluster.internal_slaves[0])
        self.assert_path_exists(self.cluster.get_master(), slave0_system_info_loc)
        self.assert_path_exists(self.cluster.get_master(), slave0_system_info_loc)

        slave0_catalog_info_loc = path.join(slave0_system_info_loc, 'catalog')
        self.assert_path_exists(self.cluster.get_master(), slave0_catalog_info_loc)
        self.assert_path_exists(self.cluster.get_master(), path.join(slave0_catalog_info_loc, 'tpch.properties'))
        self.assert_path_exists(self.cluster.get_master(), OUTPUT_FILENAME_FOR_SYS_INFO)
    ...
    def test_collect_system_info_dash_h_coord_worker(self):
        ...
        self._test_basic_system_info(actual,
                                     ...
                                     [self.cluster.get_master(), self.cluster.slaves[0]])
    ...
    def test_query_info_pa_separate_node(self):
        ...
        self.assert_path_exists(self.cluster.get_master(), query_info_file_name)
        self.assertEqual(actual, expected)
    ...
    def test_collect_logs_nonstandard_location(self):
        ...
        version = self.cluster.exec_cmd_on_host(self.cluster.get_master(), 'rpm -q --qf \"%{VERSION}\\n\" presto-server-rpm')
        ...
        self.assert_path_exists(self.cluster.get_master(), '/opt/prestoadmin/logs/presto-admin.log')

        for host in self.cluster.all_internal_hosts():
            self.assert_path_exists(self.cluster.get_master(), '/opt/prestoadmin/logs/%s/server.log' % host)
            self.assert_path_exists(self.cluster.get_master(), '/opt/prestoadmin/logs/%s/launcher.log' % host)

    def _assert_no_logs_downloaded(self):
        self._collect_logs_and_unzip()
        self.assert_path_exists(self.cluster.get_master(), '/opt/prestoadmin/logs/presto-admin.log')
        for host in self.cluster.all_internal_hosts():
            self.assert_path_exists(self.cluster.get_master(), '/opt/prestoadmin/logs/%s' % host)
            self.assert_path_removed(self.cluster.get_master(), '/opt/prestoadmin/logs/%s/*' % host)
    ...
    def test_collect_logs_multiple_server_logs(self):
        ...
        for host in self.cluster.all_internal_hosts():
            host_log_location = path.join(downloaded_logs_location, host)
            self.assert_path_exists(self.cluster.get_master(), os.path.join(host_log_location, 'server.log'))

        master_path = os.path.join(downloaded_logs_location, self.cluster.internal_master, )
        self.assert_path_exists(self.cluster.get_master(), os.path.join(master_path, 'server.log-2'))
